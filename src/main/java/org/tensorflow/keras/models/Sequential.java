...
public class Sequential extends Model<Float> {
  ...
  //                                    .addTargets(optimizer.minimize(tf, loss,
  // this.trainableVars))
  //                                    .fetch(metrics)
  //                                    .run();
  //
  ...
  //                }
  //
  //                System.out.println("Loss on epoch " + epoch + " is: loss=" + epochLoss + "
  ...
  public void fit(Ops tf, Dataset data, int epochs, int batchSize) {
    ...
    try (Session session = new Session(graph)) {
      ...
      addTargets(
          initRunner,
          new ArrayList<Operand<Float>>(
              firstLayer.initializers.values().stream()
                  .map(i -> i.getInitializerOp())
                  .collect(Collectors.toList())));
      for (Layer<Float> layer : this.layers) {
        addTargets(
            initRunner,
            new ArrayList<Operand<Float>>(
                layer.initializers.values().stream()
                    .map(i -> i.getInitializerOp())
                    .collect(Collectors.toList())));
      }
      initRunner.run();
      ...
      for (int e = 0; e < epochs; e++) {
        ...
        for (int i = 0; i < trainBatches.size(); i++) {
          ...
          try (Tensor<Float> XBatch = Tensors.create(trainBatches.get(i).X);
            ...
            List<Tensor<?>> values =
                addTargets(batchRunner, optimizer.getTargets())
                    .fetch(metricOp)
                    .fetch(lossOp)
        ...
        System.out.println(
            "(Test) Epoch " + e + " accuracy: " + epochAccuracy + "loss: " + epochLoss);
      }
    }
  }
