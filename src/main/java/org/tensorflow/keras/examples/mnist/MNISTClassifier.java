...
//            mnist.run();
    ...
    public void run() {
        try (Graph graph = new Graph()) {
            ...
            Placeholder<Float> images = tf.placeholder(Float.class, Placeholder.shape(Shape.make(-1, INPUT_SIZE)));
            ...
            Assign<Float> weightsInit = tf.assign(weights, tf.zeros(constArray(tf, INPUT_SIZE, FEATURES), Float.class));

            Variable<Float> biases = tf.variable(Shape.make(FEATURES), Float.class);
            Assign<Float> biasesInit = tf.assign(biases, tf.zeros(tf.constant(new int[] {FEATURES}), Float.class));

            Softmax<Float> softmax = tf.softmax(tf.add(tf.matMul(images, weights), biases));
            Mean<Float> crossEntropy = tf.mean(tf.neg(tf.reduceSum(tf.mul(labels, tf.log(softmax)),
                    constArray(tf, 1))), constArray(tf, 0));

            Gradients gradients = tf.gradients(crossEntropy, Arrays.asList(weights, biases));
            Constant<Float> alpha = tf.constant(LEARNING_RATE);
            ApplyGradientDescent<Float> weightGradientDescent = tf.applyGradientDescent(weights, alpha, gradients.dy(0));
            ApplyGradientDescent<Float> biasGradientDescent = tf.applyGradientDescent(biases, alpha, gradients.dy(1));
...
//                            System.out.println("Accuracy " + testCount + ": " + value.floatValue());
