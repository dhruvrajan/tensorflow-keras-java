...
public class FashionMNIST {
    ...
    private static final String LOCAL_PREFIX = "datasets/fashion_mnist/";
    ...
    public static void nioTensorFrame() throws IOException {
        FashionMNIST.download();
        // Read data files into arrays
        float[][][] trainImages = MNIST.readImages2D(Keras.kerasPath(LOCAL_PREFIX, TRAIN_IMAGES).toString());
        float[][] trainLabels = MNIST.readLabelsOneHot(Keras.kerasPath(LOCAL_PREFIX, TRAIN_LABELS).toString());
        ...
        NdArray<Float> trainImageArray = NdArrays.ofFloats(
                Shape.make(trainImages.length, trainImages[0].length, trainImages[1].length));



        NdArray trainLabelsArray = NdArrays.ofFloats(
                Shape.make(trainLabels.length, trainLabels[0].length));
